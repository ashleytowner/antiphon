...
test("public server serves only listening assets and bounded audio signaling", async () => {
  ...
  try {
    ...
    assert.deepEqual(await fetch(`${base}/api/status`).then((r) => r.json()), {
      broadcasting: false,
    });
    assert.equal((await fetch(`${base}/api/library`)).status, 404);
    assert.equal(
      (await fetch(`${base}/api/listen`, { method: "POST", body: "{}" }))
        .status,
      400,
    );
    assert.equal(
      (
        await fetch(`${base}/api/listen`, {
          method: "POST",
    ...
    assert.throws(
      () => validateOffer({ type: "offer", sdp: "v=0\r\nm=video 9\r\n" }),
      /Only audio/,
    );
  } finally {
    await server.close();
