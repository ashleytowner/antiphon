...
function updateVolume() {
  ...
  if (gain && context)
    gain.gain.setTargetAtTime(playing ? value : 0, context.currentTime, 0.015);
...
async function connect() {
  ...
  try {
    await disconnect();
    const available = await fetch("/api/status", {
      signal: AbortSignal.timeout(8000),
    }).then((r) => r.json());
    if (!started || current !== generation) return;
    if (!available.broadcasting)
      throw new Error("Waiting for the GM to start broadcasting…");
    ...
    next.ontrack = (event) => {
      ...
      void receiver.play().catch(() => {
        status.textContent =
          "Press play again to enable audio in this browser.";
      });
    ...
    next.onconnectionstatechange = () => {
      ...
      if (next.connectionState === "connected")
        status.textContent = playing
          ? "Live · connected to your GM"
          : "Paused · connected to live audio";
      if (["failed", "disconnected", "closed"].includes(next.connectionState))
        reconnect("Connection interrupted. Reconnecting…");
    };
    ...
    const response = await fetch("/api/listen", {
      ...
      body: JSON.stringify(next.localDescription),
    ...
    heartbeat = setInterval(() => {
      if (sessionId)
        void fetch(`/api/listeners/${sessionId}`, {
          method: "PUT",
          signal: AbortSignal.timeout(8000),
        })
          .then((r) => {
            if (!r.ok) reconnect("Broadcast restarted. Reconnecting…");
          })
          .catch(() => reconnect("Connection lost. Reconnecting…"));
    }, 10_000);
    // A peer can remain in connecting when a firewall drops all media packets.
    setTimeout(() => {
      if (peer === next && next.connectionState !== "connected")
        reconnect(
          "Audio connection timed out. Check the GM’s forwarded UDP ports.",
        );
    }, 15_000);
  } catch (error) {
    await disconnect();
    reconnect(
      error instanceof Error ? error.message : "Unable to connect. Retrying…",
    );
  } finally {
    connecting = false;
  }
...
toggle.addEventListener("click", async () => {
  try {
    ...
    status.textContent = `Unable to play audio: ${String(error)}`;
  }
});
