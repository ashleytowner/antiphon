...
test('streams byte ranges and handles missing or invalid ranges', async () => {
  ...
  const request = (range?: string) => new Request('https://local/track', { headers: range ? { Range: range } : {} });
  try {
    ...
    assert.equal(await (await audioResponse(file, request('bytes=-3'))).text(), '789');
    assert.equal(await (await audioResponse(file, request())).text(), '0123456789');
    ...
    assert.equal((await audioResponse(file, request('bytes=0-1,4-5'))).status, 416);
...
test('validates configurable network settings', () => {
  ...
  assert.throws(() => validateSettings({ ...defaults, udpMax: 40001 }), /32 UDP/);
  assert.throws(() => validateSettings({ ...defaults, port: 3.5 }), /integers/);
  assert.throws(() => validateSettings({ ...defaults, publicAddress: 'http://example.com' }), /IPv4/);
});
