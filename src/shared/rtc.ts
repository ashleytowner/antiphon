export async function gatherIce(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const timer = setTimeout(
      () => finish(new Error("Timed out preparing the audio connection.")),
      timeout,
    );
...
export function stereoOffer(
  offer: RTCSessionDescriptionInit,
): RTCSessionDescriptionInit {
  let sdp = offer.sdp ?? "";
  const opus = [...sdp.matchAll(/^a=rtpmap:(\d+) opus\/48000\/2\r?$/gim)].map(
    (match) => match[1],
  );
  for (const payload of opus) {
    ...
    sdp = sdp.replace(pattern, (_line, parameters: string) => {
      const values = parameters
        ...
        .split(";")
        .filter(
          (p) => !/^(?:stereo|sprop-stereo|maxaveragebitrate)=/.test(p.trim()),
        );
