...
function App() {
  ...
  return <>
    <header><div className="brand"><span className="brand-icon">♫</span><div><h1>Antiphon</h1><p>Your tabletop soundscape</p></div></div><div className="header-actions"><span className={`status-pill ${server.broadcasting ? 'live' : ''}`}>{server.broadcasting ? `● Live · ${server.listeners} listener${server.listeners === 1 ? '' : 's'}` : '○ Broadcast offline'}</span><button disabled={!settings || scanning} onClick={() => setSettingsOpen(true)}>Settings</button></div></header>
