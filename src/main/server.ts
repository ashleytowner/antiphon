...
export function validateOffer(value: unknown): { type: "offer"; sdp: string } {
  ...
  if (
    ...
    offer.sdp.length > 64_000 ||
    !offer.sdp.startsWith("v=0")
  )
    throw new Error("Invalid audio offer.");
  const media = offer.sdp.match(/^m=.+$/gm) ?? [];
  if (media.length !== 1 || !media[0].startsWith("m=audio "))
    throw new Error("Only audio is supported (one mixed audio stream).");
...
export class PlayerServer {
  ...
  async start() {
    for (const [route, file, type] of [
      ...
      ["/player.css", "player.css", "text/css"],
      ["/icon.png", "icon.png", "image/png"],
    ]) {
      this.assets.set(route, {
        body: await readFile(path.join(this.assetDirectory, file)),
        type,
      });
    }
  ...
  status(): ServerStatus {
    ...
    return {
      ...
      broadcasting: this.relay.broadcasting,
      listeners: this.relay.count,
      urls: [...new Set(hosts)].map((host) => `http://${host}:${port}`),
    };
  }
  private async handle(req: IncomingMessage, res: ServerResponse) {
    ...
    res.setHeader(
      ...
      "default-src 'self'; script-src 'self'; style-src 'self'; font-src 'self' data:; connect-src 'self'; media-src 'self' blob:; frame-ancestors 'none'",
    );
    ...
    try {
      ...
      if (req.method === "GET" && route === "/api/status") {
        reply(200, { broadcasting: this.relay.broadcasting });
        return;
      }
      ...
      if (req.method === "POST" && route === "/api/listen") {
        reply(200, await this.relay.listen(validateOffer(await jsonBody(req))));
        return;
      }
      const id = /^\/api\/listeners\/([a-f0-9-]{36})$/.exec(route)?.[1];
      if (id && req.method === "PUT") {
        reply(this.relay.touch(id) ? 200 : 404, { ok: true });
        return;
      }
      if (id && req.method === "DELETE") {
        await this.relay.remove(id);
        reply(200, { ok: true });
        return;
      }
