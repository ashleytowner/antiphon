...
function tone(seconds: number, frequency: number) {
  const rate = 48000, samples = seconds * rate;
  ...
  wave.writeUInt32LE(rate, 24); wave.writeUInt32LE(rate * 4, 28); wave.writeUInt16LE(4, 32); wave.writeUInt16LE(16, 34);
  wave.write('data', 36); wave.writeUInt32LE(samples * 4, 40);
  for (let i = 0; i < samples; i++) {
    wave.writeInt16LE(Math.round(Math.sin(i / rate * frequency * Math.PI * 2) * 6000), 44 + i * 4);
    wave.writeInt16LE(Math.round(Math.sin(i / rate * frequency * 1.5 * Math.PI * 2) * 6000), 46 + i * 4);
  }
  return wave;
}
async function instrument(page: Page) {
  await page.evaluate(() => {
    ...
    target.__peers = []; target.__gains = []; target.__analysers = []; target.__stereo = [];
    const OriginalPeer = window.RTCPeerConnection;
    window.RTCPeerConnection = class extends OriginalPeer { constructor(config?: RTCConfiguration) { super(config); target.__peers.push(this); } };
    const originalGain = AudioContext.prototype.createGain;
    AudioContext.prototype.createGain = function () { const gain = originalGain.call(this); target.__gains.push(gain); return gain; };
    ...
    AudioContext.prototype.createMediaStreamSource = function (stream) {
      ...
      const analyser = this.createAnalyser(); source.connect(analyser); target.__analysers.push(analyser);
      const splitter = this.createChannelSplitter(2); source.connect(splitter);
      const left = this.createAnalyser(), right = this.createAnalyser();
      splitter.connect(left, 0); splitter.connect(right, 1); target.__stereo.push([left, right]);
      return source;
    };
  });
}
async function energy(page: Page) {
  return page.evaluate(async () => {
    const analyser: AnalyserNode | undefined = (window as any).__analysers.at(-1);
...
test('desktop indexes, mixes, classifies and broadcasts audible live audio to a browser', async () => {
  ...
  const music = path.join(library, 'MGS Audio/music/fantasy/Adventure');
  const sfx = path.join(library, 'MGS Audio/sfx/fantasy/Spells');
  ...
  try {
    ...
    const listenerUrl = await gm.evaluate(async () => (await window.rpg.serverStatus()).urls.find(url => !url.includes('localhost')));
    await app.evaluate(async ({ BrowserWindow }, url) => {
      const listener = new BrowserWindow({ show: false, webPreferences: { backgroundThrottling: false, nodeIntegration: false, contextIsolation: true } });
      await listener.loadURL(url);
    }, listenerUrl ?? `http://127.0.0.1:${port}`);
    const player = await playerPromise;
    player.on('pageerror', error => errors.push(error.message));
    player.on('console', message => { if (process.env.RPG_TEST_DEBUG) console.log('Player:', message.text()); });
    await instrument(player);
    await player.getByRole('slider').fill('0');
    await player.getByRole('button', { name: 'Listen live' }).click();
    await expect(player.locator('#status')).toContainText('Live · connected').catch(async error => {
      console.log(await player.evaluate(() => (window as any).__peers.map((peer: RTCPeerConnection) => ({ state: peer.connectionState, ice: peer.iceConnectionState }))));
      throw error;
    });
    // GM master is zero, but transmitted audio must still decode to nonzero energy.
    await expect.poll(() => energy(player)).toBeGreaterThan(0.0001).catch(async error => {
      for (const page of [gm, player]) console.log(await page.evaluate(async () => {
        const reports: any[] = [];
        for (const peer of (window as any).__peers) (await peer.getStats()).forEach((s: any) => { if (['inbound-rtp', 'outbound-rtp', 'media-source'].includes(s.type)) reports.push(s); });
        return reports;
      }));
      throw error;
    });
    ...
    await expect.poll(() => player.evaluate(() => {
      ...
      const a = new Float32Array(left.fftSize), b = new Float32Array(right.fftSize);
      left.getFloatTimeDomainData(a); right.getFloatTimeDomainData(b);
      let xy = 0, xx = 0, yy = 0;
      for (let i = 0; i < a.length; i++) { xy += a[i] * b[i]; xx += a[i] ** 2; yy += b[i] ** 2; }
      return xx > 0 && yy > 0 ? Math.abs(xy / Math.sqrt(xx * yy)) : 1;
    })).toBeLessThan(0.5);
    await gm.getByRole('slider', { name: 'Volume for Test Adventure', exact: true }).fill('0');
    await expect.poll(() => energy(player)).toBeLessThan(0.000001);
    await gm.getByRole('slider', { name: 'Volume for Test Adventure', exact: true }).fill('0.5');
    ...
    expect(await player.evaluate(() => (window as any).__gains[0].gain.value)).toBe(0);
    await expect.poll(() => energy(player)).toBeGreaterThan(0.0001);
    await player.getByRole('button', { name: 'Listen live' }).click();
    await expect(player.locator('#status')).toContainText('Live · connected');
    // Music remains playing across its two-second loop boundary.
    await expect(gm.locator('.channel.playing')).toHaveCount(1);
    await gm.getByRole('button', { name: 'Pause Test Adventure', exact: true }).click();
    await expect(gm.locator('.channel.playing')).toHaveCount(0);
    await gm.getByRole('button', { name: 'Play Test Adventure', exact: true }).click();
    await gm.locator('.channel').first().getByRole('checkbox', { name: 'Loop' }).uncheck();
    await expect(gm.getByRole('button', { name: 'Play Test Adventure', exact: true })).toBeVisible();
    await gm.locator('.channel').first().getByRole('checkbox', { name: 'Loop' }).check();
    await gm.getByRole('button', { name: 'Play Test Adventure', exact: true }).click();
    ...
    await expect(gm.getByRole('button', { name: 'Play Test Spell', exact: true })).toBeVisible();
    await expect(gm.locator('.channel.playing')).toHaveCount(1);
