...
export const defaults: Settings = { libraryRoot: '', port: 3000, udpMin: 40000, udpMax: 40031, publicAddress: '', stunUrl: '', turnUrl: '', turnUsername: '', turnCredential: '' };
export function validateSettings(value: Settings): Settings {
  if (!value || typeof value !== 'object') throw new Error('Invalid settings.');
  for (const field of ['libraryRoot', 'publicAddress', 'stunUrl', 'turnUrl', 'turnUsername', 'turnCredential'] as const) {
    if (typeof value[field] !== 'string' || value[field].length > 4096) throw new Error(`Invalid ${field}.`);
  }
  for (const key of ['port', 'udpMin', 'udpMax'] as const) if (!Number.isInteger(value[key]) || value[key] < 1024 || value[key] > 65535) throw new Error('Ports must be integers between 1024 and 65535.');
  if (value.udpMax - value.udpMin < 31) throw new Error('Provide at least 32 UDP ports for concurrent listeners.');
  if (value.publicAddress && !isIPv4(value.publicAddress)) throw new Error('Public address must be an IPv4 address (without http:// or a port).');
...
export async function storeSettings(filename: string, value: Settings) {
  await writeFile(`${filename}.tmp`, JSON.stringify(validateSettings(value), null, 2), { mode: 0o600 });
