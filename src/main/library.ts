...
export class Library {
  ...
  query(root: string, query: LibraryQuery): LibraryResult {
    ...
    for (const field of ['type', 'era', 'genre'] as const) if (query[field]) { where.push(`${field} = ?`); args.push(query[field]!); }
    if (query.reviewOnly) where.push('needsReview = 1');
    for (const word of (query.search ?? '').trim().split(/\s+/).filter(Boolean).slice(0, 20)) {
      where.push("(title || ' ' || album || ' ' || relativePath || ' ' || type || ' ' || era || ' ' || genre) LIKE ? ESCAPE '\\'");
  ...
  facets(root: string): Facets {
    ...
    const counts = this.db.prepare('SELECT count(*) AS total, coalesce(sum(needsReview),0) AS review, coalesce(sum(missing),0) AS missing FROM tracks WHERE root = ?').get(root)!;
  ...
  file(root: string, id: number): string | undefined {
    const row = this.db.prepare('SELECT relativePath FROM tracks WHERE root=? AND id=? AND missing=0').get(root, id);
    return row ? path.join(root, String(row.relativePath)) : undefined;
  }
  async scan(root: string, progress: (value: ScanProgress) => void = () => {}) {
    if (!root || !path.isAbsolute(root)) throw new Error('Select an absolute library folder first.');
    ...
    const walk = async (directory: string) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) await walk(filename);
          ...
          files.push({ relativePath: path.relative(root, filename), size: info.size, mtime: info.mtimeMs });
          if (Date.now() - lastUpdate > 150) { lastUpdate = Date.now(); progress({ phase: 'scanning', count: files.length, message: path.relative(root, filename) }); }
        }
      }
    };
    await walk(root);
    const matches = titleMatches(files.map(f => f.relativePath));
    progress({ phase: 'saving', count: files.length, message: 'Saving classifications…' });
    const upsert = this.db.prepare(`INSERT INTO tracks (root,relativePath,title,album,type,era,genre,needsReview,reason,size,mtime)
    ...
    progress({ phase: 'done', count: files.length, message: `Indexed ${files.length.toLocaleString()} audio files` });
    return files.length;
  }
}
