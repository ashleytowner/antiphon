...
export function normalizedTitle(value: string): string {
  return value
    ...
    .toLowerCase()
    .replace(/[\u200b-\u200d\ufeff]/g, "")
    .replace(/\s*\((?:loop|[a-f0-9]{6})\)/g, "")
    .replace(/[^\p{L}\p{N}]/gu, "");
}

export function structuredClassification(
  relativePath: string,
): Classification | undefined {
  const parts = relativePath.split(/[\\/]/);
  // Accept both <type>/<era>/<genre>/<file> and collections which wrap that
  // hierarchy in one provider directory.
  const typeIndex = ["music", "ambience", "sfx"].includes(
    parts[0]?.toLowerCase(),
  )
    ? 0
    : ["music", "ambience", "sfx"].includes(parts[1]?.toLowerCase())
      ? 1
      : -1;
  ...
  return {
    type,
    era: parts[typeIndex + 1].toLowerCase(),
    genre: parts[typeIndex + 2],
    needsReview: false,
...
const genreRules: [RegExp, string][] = [
  ...
  [
    /dungeon|cave|cavern|sewer|underground|below the surface|subterranean|mines?\b/,
    "Dungeon",
  ],
  [
    /gothic|haunted|graveyard|crypt|vampire|curse|horror|nightmare|creepy/,
    "Gothic",
  ],
...
export function classify(
  relativePath: string,
  matches = new Map<string, Classification>(),
): Classification {
  const parts = relativePath.split(/[\\/]/);
  ...
  if (matching && matching.type === type)
    return {
      ...matching,
      reason: "Title matches a consistently classified structured track",
      needsReview: true,
    };
  const text = `${album} ${title}`
    ...
    .replace(/[\u200b-\u200d\ufeff]/g, "")
    .toLowerCase();
  let era = /sci.?fi|space|stardust|starship|cyber|robot/.test(text)
    ? "scifi"
    : /modern/.test(text)
      ? "modern"
      : /middle.?eastern/.test(text)
        ? "middle-eastern"
        : "fantasy";
  const trackGenre = genreRules.find(([pattern]) =>
    pattern.test(title.toLowerCase()),
  )?.[1];
  const albumGenre = genreRules.find(([pattern]) =>
    pattern.test(album.toLowerCase()),
  )?.[1];
  // The themed music collections describe genre more reliably than individual titles.
  const genre =
    type === "Music" && /music collection/i.test(album) && albumGenre
      ? albumGenre
      : (trackGenre ??
        albumGenre ??
        (type === "Ambience" ? "Nature" : "Exploration"));
  if (
    type === "Ambience" &&
    era === "fantasy" &&
    [
    ...
    !/elven|magic|haunted|fey|monster|dream|evil|hell|plane/.test(
      title.toLowerCase(),
    )
  )
    era = "generic";
  ...
  for (const file of paths) {
    ...
    if (
      ...
      (previous.type !== category.type ||
        previous.era !== category.era ||
        previous.genre !== category.genre)
    )
      ambiguous.add(key);
