...
const peer = (local = false) =>
  new RTCPeerConnection({
    ...
    ...(local
      ...
      : {}),
    codecs: { audio: [useOPUS({ parameters: "stereo=1;sprop-stereo=1" })] },
  });
async function until(check: () => boolean, timeout = 10000) {
  const deadline = Date.now() + timeout;
  while (!check()) {
    if (Date.now() > deadline) throw new Error("Timed out waiting for WebRTC");
    await new Promise((r) => setTimeout(r, 20));
  }
}
test(
  "relays live Opus to eight listeners, supports late joins, and releases sessions",
  { timeout: 90000 },
  async () => {
    ...
    try {
      ...
      await publisher.setRemoteDescription(
        await relay.publish(publisher.localDescription!),
      );
      await until(
        () => relay.broadcasting && publisher.connectionState === "connected",
      );
      let sequence = 0;
      timer = setInterval(() => {
        sequence++;
      ...
      await Promise.all(
        Array.from({ length: 8 }, async (_, i) => {
          ...
          listener.onTrack.subscribe((track) =>
            track.onReceiveRtp.subscribe((packet) => {
              counts[i]++;
              timestamps[i] = packet.header.timestamp;
            }),
          );
          ...
          for (const candidate of answer.sdp.matchAll(
            /a=candidate:.*? udp \d+ \S+ (\d+)/gi,
          ))
            assert.ok(
              Number(candidate[1]) >= testSettings.udpMin &&
                Number(candidate[1]) <= testSettings.udpMax,
            );
      ...
      assert.ok(
        opus.length > 4,
        "Opus output subscribers receive the live mix",
      );
      assert.deepEqual(opus[0], Buffer.from([0xf8, 0xff, 0xfe]));
      assert.equal(relay.count, 8);
      assert.ok(
        Math.max(...timestamps) - Math.min(...timestamps) <= 4800,
        "listeners receive the same live timeline",
      );
      ...
      await assert.rejects(
        relay.listen(listeners[0].localDescription!),
        /not started/,
      );
    } finally {
      unsubscribeOpus();
